package com.alien.admin.config.interceptor;
import cn.hutool.core.util.StrUtil;
import com.alien.admin.common.annotation.RateLimiter;
import com.alien.admin.common.limit.RedisRaterLimiter;
import com.alien.admin.common.util.IpInfoUtil;
import com.alien.admin.config.properties.AlienIpLimitProperties;
import com.alien.admin.config.properties.AlienLimitProperties;
...
public class LimitRaterInterceptor extends HandlerInterceptorAdapter {
    ...
    @Autowired
    private AlienLimitProperties limitProperties;

    @Autowired
    private AlienIpLimitProperties ipLimitProperties;
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if(ipLimitProperties.getEnable()) {
            ...
            if (StrUtil.isBlank(token1)) {
                throw new AlienException("你手速怎么这么快，请点慢一点");
            }
        }
        ...
        if(limitProperties.getEnable()){
            ...
            if (StrUtil.isBlank(token2)) {
                throw new AlienException("当前访问总人数太多啦，请稍后再试");
            }
        }
        ...
        try {
            ...
            if (rateLimiter != null) {
                ...
                if (StrUtil.isBlank(token3)) {
                    throw new AlienException("当前访问人数太多啦，请稍后再试");
                }
            }
        }catch (AlienException e){
            throw new AlienException(e.getMsg());
        }catch (Exception e){
